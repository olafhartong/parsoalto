  ...
  for firewall in $fw; do
    for host in $vsys; do
     for url in $config; do
      ...
      curl -s -k "https://$firewall/api/?type=config&action=get&key=$key&xpath=/config/devices/entry\[@name='localhost.localdomain'\]/vsys/entry\[@name='$host'\]/$url" | xmllint --format --recover -  > $location$prefix-$ipfilename-$host.$fileurl.$timestamp.xml
    done
  done
